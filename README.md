...
본 프로젝트는 데이터 전처리부터 실전 주행 영상 검증까지 총 3단계의 파이프라인으로 구성되어 있습니다. 아래의 Jupyter Notebook 파일들을 순서대로 실행하여 결과를 재현할 수 있습니다.

### 1. Data Preprocessing (데이터 전처리 및 분할)
블랙박스 화각의 특성을 고려하여 불필요한 영역을 크롭(Crop)하고, 총 2,009장의 데이터셋을 Train(8) : Val(2) 비율로 자동 분할합니다.
* **Run:** `프로젝트4조 데이터 크롭.ipynb`

### 2. Train Model (YOLO11 모델 학습 및 평가)
전처리된 도로 파손 데이터셋(Pothole, Crack, Manhole 3개 클래스)을 활용하여 YOLO11 아키텍처 기반의 학습을 진행하고, mAP 등 정량적 평가지표를 도출합니다.
* **Run:** `AI 프로젝트 1차 2차 실험.ipynb`

### 3. Inference on Video (실전 주행 영상 객체 추적)
학습이 완료된 최적 가중치(`best.pt`)를 불러와, 실제 블랙박스 주행 영상(맑은날.mp4 등)에서 도로 파손 객체를 실시간으로 탐지하고 추적(Tracking)합니다. 
